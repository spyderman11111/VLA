...
class SimpleGraspNode(Node):
    def __init__(self):
        ...
        self.policy.config.input_features = {
            ...
            "goal": PolicyFeature(type=FeatureType.TEXT)
        }
        ...
        self.home_positions = [
            -1.4959, 0.1893, -0.2826, -1.5792, 0.0392, 1.5944
        ]
        self.go_home()
    ...
    def go_home(self):
        ...
        self.get_logger().info("Going to initial position with velocity control...")
    ...
    def try_infer(self):
        ...
        try:
            ...
            joints = np.asaPI0Policyrray(out)[:6]
            self.send_traj(joints)
            self.get_logger().info(f"Action: {joints.tolist()}")
            self.create_timer(4.0, self.reset_and_shutdown)
        except Exception as e:
            self.get_logger().error(f"Policy failed: {e}")
            rclpy.shutdown()
    ...
    def send_traj(self, positions):
        ...
        msg.points.append(point)
        self.pub.publish(msg)
        self.get_logger().info("Action published with velocity control.")

    def reset_and_shutdown(self):
        self.go_home()
        self.get_logger().info("Returning to home and shutting down.")
        self.destroy_node()
        rclpy.shutdown()
